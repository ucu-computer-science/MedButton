...
void MarkerModel::change(int id, const QGeoCoordinate & coordinate)
{
    if (id==0 && m_current.isValid()) {
        m_current = coordinate;
        Q_EMIT currentChanged();
        return;
    }
    if(m_markers.length() < id || m_markers.isEmpty())
        return;
    qDebug() << m_markers.length();
    m_markers[id-1] = coordinate;
}
